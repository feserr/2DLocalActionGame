...
{
	public class JoystickLeft : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
	{
		...
		public string horizontalAxisName = "LeftJoystickHorizontal"; // The name given to the horizontal axis for the cross platform input
		public string verticalAxisName = "LeftJoystickVertical"; // The name given to the vertical axis for the cross platform input
        ...
        public int m_currentTouchIDLeft;
        private JoystickRight m_joystickRight;

        public int CurrentTouchID
        { get { return m_currentTouchIDLeft; } }
        ...
        void Start()
        {
            ...
            m_currentTouchIDLeft = -1;

            m_joystickRight = GameObject.Find("RightMobileJoystick").GetComponent<JoystickRight>();
        }
        ...
        void Update()
        {
            for (var i = 0; i < Input.touchCount; ++i)
            {
                if(i != -1)
                {
                    if(m_currentTouchIDLeft == -1 &&
                        (m_joystickRight.CurrentTouchID == -1 || m_joystickRight.CurrentTouchID != i))
                    {
                        Touch t_touch = Input.GetTouch(i);

                        if(t_touch.position.x < (Screen.width / 2))
                        {
                            m_currentTouchIDLeft = i;
                            break;
                        }
                    }
                }
            ...
            if(m_currentTouchIDLeft != -1)
            {
                Touch touch = Input.GetTouch(m_currentTouchIDLeft);
